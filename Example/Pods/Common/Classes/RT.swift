...
public class RT: NSObject {
    ...
    private var appID: String {
        return Params.named("S.RT.AppID").string ?? _appID
    }
    ...
    public var persistentKey = "Rating.1" // 记录用户是否评论过的 UserDefault Key
    public var minTime: TimeInterval {
        let param = Params.named("S.RT.minTime")
        if param != JSON.null {
    ...
    public func showAlert(from controller: UIViewController, dataKey: String, force: Bool = false, action: RatedCompleted? = nil) {
        self.showAlert(from: controller, data: Params.named(dataKey), force: force, action: action)
    }
...
//    private func postEvent(ok: Bool) {
//        let countryCode = NSLocale.current.regionCode ?? "OTHER"
//        if ok {
//            LeanCloud.event("rating-\(countryCode)-ok")
//            LeanCloud.event("rating-\(countryCode)", label: "ok")
//        } else {
//            LeanCloud.event("rating-\(countryCode)", label: "cancel")
//        }
//    }
//    
    // 移除备注
